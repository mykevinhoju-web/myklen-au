...
import type { KoPackageSlug } from '@/lib/site-content-ko'
...
export function PackageCardKo({ pkg, heroTint = false }: Props) {
  const copy = koSite.packages.bySlug[pkg.slug as KoPackageSlug]
  const name = copy?.name ?? pkg.name
  const description = copy?.description ?? pkg.description
  const highlights = copy?.highlights ?? pkg.highlights
  ...
  return (
    ...
    >
      ...
      <p className="type-lead mt-4 flex-1 text-base">{description}</p>
